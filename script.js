document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 4. Lógica de Generación de Boletos y PDF ---
    ...
    const generateUniqueId = (name) => {
        // Genera un ID simple basado en el nombre y un timestamp
        const initial = name.trim().toUpperCase().charAt(0);
        const timestamp = Date.now().toString().slice(-4);
        // Usamos solo los primeros 4 dígitos del nombre para más variabilidad
    ...
    const generateTicketHtml = (name) => {
        ...
        const now = new Date();
        const generationTime = now.toLocaleDateString('es-ES', { 
            year: 'numeric', 
            month: 'short', 
            day: 'numeric',
            hour: '2-digit',
        ...
        return `
            <div class="magic-ticket">
                ...
                <div class="ticket-content">
                    <i class="fas fa-scroll ticket-icon"></i>
                    <p class="elegant-text ticket-header">Pase para el Gran Baile</p>
                    <p class="cinzel-font ticket-name">${name.toUpperCase()}</p>
                    <div class="ticket-details">
                        <div class="detail-item">
                            <p class="detail-label">FECHA DEL EVENTO</p>
                            <p class="detail-value">SÁBADO 21 FEB, 2026</p>
                        </div>
                        <div class="detail-item">
                            <p class="detail-label">INGRESO AL SALÓN</p>
                            <p class="detail-value">9:00 PM</p>
                        </div>
                    </div>
                    <div class="ticket-footer">
                        <p class="ticket-id">CÓDIGO DE ACCESO: ${id}</p>
                        <p class="ticket-generation-info">Generado: ${generationTime}</p>
                    </div>
                </div>
            </div>
        `;
    ...
    // Asignar el evento al botón de impresión al cargar la página
    if (printButton) {
    ...
    rsvpForm.addEventListener('submit', (e) => {
        ...
        // 1. Limpiar y separar los nombres por coma
        ...
        // 2. Generar el HTML de todos los boletos
        let ticketsHtml = namesArray.map(name => generateTicketHtml(name)).join('');

        // 3. Mostrar los boletos y la sección
        ticketsContainer.innerHTML = ticketsHtml;
        ticketDisplay.style.display = 'block';
        
        // Desplazarse a la sección de boletos
        ticketDisplay.scrollIntoView({ behavior: 'smooth', block: 'start' });
        
        // Opcional: Deshabilitar el formulario después de generar
